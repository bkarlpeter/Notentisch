(function initializeBoardPresets() {
    ...
    function generateCustomCSS(preset) {
        ...
        return [
            ...
            `body[data-board-preset="${id}"] .quadrant { background: ${qc}; }`,
            `body[data-board-preset="${id}"] .center-hole { background: var(${v}) center/320px auto repeat; gap:0; border:none; padding:0; box-shadow:0 0 50px rgba(0,0,0,.9); isolation:isolate; }`,
            `body[data-board-preset="${id}"] .center-hole::before { content:""; position:absolute; inset:calc(-1 * var(--center-frame-width)); border-radius:calc(20px + var(--center-frame-width)); background: linear-gradient(135deg,rgba(255,255,255,.18) 0%,rgba(0,0,0,.22) 100%), var(${v}) center/320px auto repeat; box-shadow:0 16px 30px rgba(0,0,0,.42),0 0 0 1px rgba(30,30,30,.7),inset 0 0 0 1px rgba(255,255,255,.3); z-index:-1; pointer-events:none; }`,

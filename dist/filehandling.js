...
async function handleLoadButton() {
    if (xmlData) {
        const statusEl = document.getElementById('commandStatus');
        if (statusEl) statusEl.textContent = 'XML bereits geladen';
        return;
    }
...
let searchKeyboardHooksBound = false;
let searchOverlayHistoryPushed = false;
...
function requestSearchKeyboard(action) {
    const safeAction = action === 'close' ? 'close' : 'open';
    const modeParam = safeAction === 'open' ? '&mode=fallback' : '';
    try {
        fetch('/__keyboard__?action=' + encodeURIComponent(safeAction) + modeParam, {
            method: 'POST',
...
function bindSearchKeyboardAutoCloseHooks() {
    if (searchKeyboardHooksBound) return;
    searchKeyboardHooksBound = true;
    ...
    const closeIfVisible = () => {
        ...
        requestSearchKeyboard('close');
    };

    window.addEventListener('popstate', closeIfVisible);
    window.addEventListener('beforeunload', () => requestSearchKeyboard('close'));
...
function openSearchKeyboardCompact(event) {
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }
    ...
    if (!input) return;
    // Search-Input zwingt in der Regel die kompaktere Such-Tastatur auf Touch-Geräten.
    input.setAttribute('inputmode', 'search');
    input.setAttribute('enterkeyhint', 'search');
    showSearchKeyboard(input);

    let browserKeyboardShown = false;
    try {
        if (navigator.virtualKeyboard && typeof navigator.virtualKeyboard.show === 'function') {
            navigator.virtualKeyboard.show();
            browserKeyboardShown = true;
        }
    } catch {}

    // Windows-Fallback immer zusätzlich versuchen: manche Browser melden Erfolg,
    // blenden aber auf Desktop dennoch keine sichtbare Tastatur ein.
    fetch('/__open_keyboard__?mode=compact', {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain;charset=UTF-8' },
        body: 'open'
    })
        ...
        .then(data => {
            ...
            if (!browserKeyboardShown) {
                setSearchMessage('Tastatur konnte nicht gestartet werden.');
            }
        })
        .catch(() => {
            if (!browserKeyboardShown) {
...
function openSearchOverlay() {
    ...
    bindSearchKeyboardAutoCloseHooks();
    if (!searchOverlayHistoryPushed) {
        try {
            history.pushState({ __searchOverlay: true }, '');
            searchOverlayHistoryPushed = true;
        } catch {}
    }
...
function closeSearchOverlay() {
    ...
    setTimeout(() => requestSearchKeyboard('close'), 120);
    if (searchOverlayHistoryPushed) {
        searchOverlayHistoryPushed = false;
        try {
            history.back();
        } catch {}
    }
